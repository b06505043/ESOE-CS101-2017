...
def modelPrinter(model):
    turingModelSTR = '''Turing machine是一種抽象的數學計算模型，由四個部分組成:
    1.TAPE:是一條可向右伸展的無限長紙帶，且紙帶被劃分為連續的小格，每一格上都註記著符號。
    2.HEAD:一個讀寫頭，可在TAPE上左右移動讀取每格上的符號。
    3.TABLE:一套控制的規則，會根據當前機器的狀態及讀寫頭所指的格子上的符號來確定讀寫頭下一步的動作，並改變狀態暫存器的值，令機器進入一個新的狀態。
    4.狀態暫存器:用來儲存Turing machine當前的狀態。
    '''
